    ...
    <Link to={`/spots/${spot._id}`}>
      <div className="bg-white shadow-lg rounded-lg overflow-hidden pb-4">
        {spot.imageUrls.length > 0 ? (
          <img
            className="w-full h-48 object-cover object-center mb-1"
        ...
        {spot.available ? (
          <p className="px-4 py-1 bg-green-600 text-white rounded-sm">
            Available
          </p>
        ) : (
          <p className="px-4 py-1 bg-red-600 text-white rounded-sm">
            Unavailable
          </p>
        )}
        ...
        <div className="flex items-center justify-between p-2">
          ...
          <div className="">
            ...
            <p className="text-blue-900"> {spot.distance} km away</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-4 mt-2 p-2">
          {spot.features.map((feature, index) => (
            <span
              key={index}
              className="bg-blue-500 rounded-lg px-4 py-1 text-sm"
            >
              {feature}
            </span>
          ))}
